(function($, window, undefined) {
	...
	function main() {
		...
		$('#expr').val('A&!B&!Bin|!A&B&!Bin |!A&!B&Bin|A&B&Bin');
		$('#expr').keyup();
	}
	that.main = main;
	...
	var parse = (function() {
		...
		function consumeToken(expected) {
			...
			if (expected && curTok !== expected) {
				throw new SyntaxError(
					'Did not encounter expected token. Expected: "' + expected + '" Actual: "' + curTok + '".'
				);
			}
		...
		return function(tok) {
			...
			if (pos < tokens.length) {
				...
				debug(tokens[pos]);
				throw new SyntaxError(
					'Could not consume all tokens. ' + 'Remaining tokens: ' + tokens.slice(pos, tokens.length)
				);
			}
